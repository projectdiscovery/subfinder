...
func (a *Agent) buildMultiRateLimiter(ctx context.Context, globalRateLimit int, rateLimit *subscraping.CustomRateLimit) (*ratelimit.MultiLimiter, error) {
	...
	for _, source := range a.sources {
		...
		var sourceRateLimit subscraping.RateLimitInfo
		var hasCustom bool
		if rateLimit != nil {
			sourceRateLimit, hasCustom = rateLimit.Custom.Get(strings.ToLower(source.Name()))
		}

		if hasCustom {
			// Use source-specific rate limit
			...
			// Use global rate limit if no source-specific rate limit is set
