// Package shodandomain logic
package shodandomain
...
// The page carries other lists, so take the subdomain list by its id first.
var (
	reSubdomainList = regexp.MustCompile(`(?s)<ul id="subdomains">(.*?)</ul>`)
	reSubdomain     = regexp.MustCompile(`<li>([^<]*)</li>`)
)
...
func (s *Source) Run(ctx context.Context, domain string, session *subscraping.Session) <-chan subscraping.Result {
	...
	go func() {
		...
		// The service answers for the registrable domain whatever host it is
		// given, so the labels it returns belong to that root rather than to
		// domain. Appending them to domain instead would invent names such as
		...
		s.requests++
		resp, err := session.SimpleGet(ctx, fmt.Sprintf("https://www.shodan.io/domain/%s", root))
		if err != nil {
		...
		list := reSubdomainList.FindStringSubmatch(string(body))
		if list == nil {
			return
		}

		for _, match := range reSubdomain.FindAllStringSubmatch(list[1], -1) {
